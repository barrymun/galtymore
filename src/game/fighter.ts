...
import { Coords, DirectionFaced, SpriteAnimation } from "utils/types";
import { Colour, defaultAttackDamage, defaultHealth } from "utils/constants";
...
export class Fighter extends Sprite {
  ...
  private switchSpriteState = (state: SpriteAnimation): void => {
    ...
    if (
      this.getImage().src === this.getSprites().attack[this.getShouldFlip() ? "flippedImage" : "image"].src &&
    ...
    if (
      this.getImage().src === this.getSprites().takeHit[this.getShouldFlip() ? "flippedImage" : "image"].src &&
  ...
  private drawHealthBarInfo = (): void => {
    ...
    this.getContext().fillText(
      `${this.getHealth()}%`,
      this.getPosition().x +
        this.getHealthBarOffset().x -
        this.remainingHealthIndicatorWidth +
        this.remainingHealthIndicatorWidth / 2,
